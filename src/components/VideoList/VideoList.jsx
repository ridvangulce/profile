...
function VideoPlayer() {
  ...
  const renderPortfolioItem = (item) => (
    <div key={item.id} className="relative">
      <div className="bg-gray-200 rounded-lg overflow-hidden shadow-md hover:shadow-xl transition duration-300 cursor-pointer" onClick={() => handleVideoToggle(item.id)}>
        {coverUrls[item.id] ? (
          <img src={coverUrls[item.id]} alt={item.name} className=" object-cover object-center" />
        ) : (
